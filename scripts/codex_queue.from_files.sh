...
for i in "${!FILES[@]}"; do
  ...
  timeout 40m codex -m gpt-5 --full-auto "$(cat "$f")" || { echo "falha codex em $f"; rollback; continue; }
  if validate; then commit_if "$msg"; git push -u origin feature/codex-overnight || true; else echo "validação falhou em $f"; rollback; fi
  sleep 3
done
